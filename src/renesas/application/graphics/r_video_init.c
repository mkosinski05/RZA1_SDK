 ...
 * @retval        vdc5_error_t
 ******************************************************************************/
vdc_error_t video_init_vdc5(const vdc_channel_t vdc5_ch)
{
    vdc_error_t error;
...
#if ((INPUT_SELECT == ANALOG_NTSC)||(INPUT_SELECT == ANALOG_PAL))
    {
        ...
        digital = NULL;
        error = R_RVAPI_VideoControlVDC5 (vdc5_ch, digital);

        /* Wait for Color system determination */
        osDelay (500);

        /* VDEC init */
        if (VDC_CHANNEL_0 == vdc5_ch)
        {
    ...
    if (VDC5_OK == error)
    {
        R_RVAPI_VideoPortSettingVDC5 (vdc5_ch, VDC5_CameraPortSetting);
    }
#endif
