...
#error ERROR: Invalid INPUT_SELECT.
    ...
    if (error == VDC_OK)
    {
        ...
        /* Capture area */
        v_cnf.cap_area.vs = CAP_VDC5_VS;
        v_cnf.cap_area.vw = CAP_VDC5_VW;
        v_cnf.cap_area.hs = CAP_VDC5_HS;
        v_cnf.cap_area.hw = CAP_VDC5_HW;
        ...
        v_cnf.write_format = CAP_VDC5_WFORMAT; /* Write format               */
        v_cnf.write_swap = VDC_WR_RD_WRSWA_NON; /* Write swap 8bit/16bit/32bit */
        v_cnf.write_fb_vw = CAP_VDC5_SIZE_VW; /* 1/1 Capture (480u/2u) -> scale down -> 240u out */
        v_cnf.write_fb_hw = CAP_VDC5_SIZE_HW; /* 1/2 Capture (720u*2u) -> scale down -> 720u out */
        v_cnf.res_inter = CAP_VDC5_FIELD; /* Field operating mode select */
