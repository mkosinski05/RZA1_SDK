 ...
 * Arguments : *p_dma_info_drv - Pointer of gb_info_drv is returned
 * Return Value : None
 ******************************************************************************/
 ...
 * Arguments : *p_dma_init_param - Pointer to init parameters
 * Return Value : IOIF_ESUCCESS - Operation successful
 *                OS error num - Registering handler failed
 ******************************************************************************/
int_t DMA_Initialize(const dma_drv_init_t * const p_dma_init_param)
{
    int_t retval = IOIF_ESUCCESS;
    int_t ch_count;
    uint32_t error_code;
    ...
    /* initialise channel management information */
    for (ch_count = DMA_START_CHANNEL; ch_count < DMA_CH_NUM; ch_count++)
    {
        /* set channel number */
        ...
        /* initialise next DMA setting flag */
        ...
        /* set common register for channel 0 - 7 */
    ...
    if (IOIF_ESUCCESS == retval)
    {
        /* DMA end interrupt handler register */
        init_check_flag = false;
        ch_count = 0;

        while (!init_check_flag)
        {
            error_code = R_INTC_RegistIntFunc(gb_info_drv.info_ch[ch_count].end_irq_num, gb_dma_int_handler_table[ch_count]);
            error_code = R_INTC_SetPriority(gb_info_drv.info_ch[ch_count].end_irq_num, 31);

            /* 0 is no error on InterruptHandlerRegister() */
            if (DRV_SUCCESS != error_code)
            {
        ...
        {
            error_code = R_INTC_RegistIntFunc(gb_info_drv.err_irq_num, &R_DMA_ErrInterruptHandler);
            error_code = R_INTC_SetPriority(gb_info_drv.err_irq_num, 31);
    ...
    if (DRV_SUCCESS == retval)
    {
        ...
        for (ch_count = DMA_START_CHANNEL; ch_count < DMA_CH_NUM; ch_count++)
        {
            ...
            R_INTC_SetConfiguration(gb_info_drv.info_ch[ch_count].end_irq_num, INTC_EDGE_TRIGGER);
            R_INTC_SetPriority(gb_info_drv.info_ch[ch_count].end_irq_num, 28);
        }

        /* set DMA error interrupt level (set edge trigger, 1-N model) */
        R_INTC_SetConfiguration(gb_info_drv.err_irq_num, INTC_EDGE_TRIGGER); 

        /* DMA error interrupt enable */
        R_INTC_Enable(gb_info_drv.err_irq_num);
    }
...
int_t DMA_UnInitialize(void)
{
    ...
    ch_count = DMA_START_CHANNEL;

    /* disable DMA interrupts */
    for (ch_count = DMA_START_CHANNEL; ch_count < DMA_CH_NUM; ch_count++)
    {
        R_INTC_Disable(gb_info_drv.info_ch[ch_count].end_irq_num);
        R_INTC_UnRegistIntFunc(gb_info_drv.info_ch[ch_count].end_irq_num);
        gb_info_drv.info_ch[ch_count].ch_stat = DMA_CH_UNINIT;
    }

    /* disable DMA error interrupt */
    R_INTC_Disable(gb_info_drv.err_irq_num);
        
    /* unregister DMA interrupt error handler */
    R_INTC_UnRegistIntFunc(gb_info_drv.err_irq_num);
 ...
 * Arguments : channel - Open channel number
 * Return Value : None
 ******************************************************************************/
static void DMA_OpenChannel(const int_t channel)
{
 ...
 * Arguments : None
 * Return Value :   channel - Open channel number
 *                  error code -
 *                     IOIF_EMFILE : When looking for a free channel, but a free
 *                              channel didn't exist
 ******************************************************************************/
int_t DMA_GetFreeChannel(void)
{
    int_t retval = IOIF_EFAULT;
 ...
 * Return Value :   channel - Open channel number
 *                  error code -
 *                     IOIF_EBUSY : It has been allocated already in channel
 *                     IOIF_ENOTSUP : Channel status is DMA_CH_UNINIT
 *                     IOIF_EFAULT: Channel status is besides the status defined in
 *                             dma_stat_ch_t
 ******************************************************************************/
int_t DMA_GetFixedChannel(const int_t channel)
{
    ...
    /* prevent access to channels below DMA_START_CHANNEL */
    if (channel < DMA_START_CHANNEL)
    {
        return (IOIF_EFAULT);
    }
 ...
 * Arguments : channel - Close channel number
 * Return Value : None
 ******************************************************************************/
 ...
 * Arguments : channel - Set up channel number
 *             *p_ch_setup - Set up parameters
 *             *p_ch_cfg - DMA channel config table parameters
 *             *reqd - set value for REQD bit on CHCFG
 * Return Value : None
 ******************************************************************************/
                  ...
                  const dma_ch_cfg_t * const p_ch_cfg, const uint32_t reqd)
{
    ...
    /* set DMA transfer parameter to DMA channel information */
 ...
 * Arguments : channel - Set address channel number
 *             *p_dma_data - DMA transfer address parameter set
 * Return Value : None
 ******************************************************************************/
 ...
 * Arguments : channel - Set address channel number
 *             *p_dma_data - DMA transfer address parameter set
 *             next_register_set - Number of next register set
 * Return Value : None
 ******************************************************************************/
void DMA_SetData(const int_t channel, const dma_trans_data_t * const p_dma_data, const uint32_t next_register_set)
{
    /* prevent access to channels below DMA_START_CHANNEL */
    if (channel < DMA_START_CHANNEL)
    {
    ...
    if (0U == next_register_set)
    {
        ...
        /* set DAM transfer address to register */
    ...
    {
        /* set DMA transfer address parameters to next register set1 */
...
void DMA_SetNextData(const int_t channel, const dma_trans_data_t * const p_dma_data)
{
    ...
    /* set flag which indicates that next DMA transfer set already */
    gb_info_drv.info_ch[channel].next_dma_flag = true;

    /* auto restart continuous DMA */
...
/******************************************************************************
 * Function Name: DMA_SoftwareTrigger
 * Description : Software trigger DMA transfer
 * Arguments : channel - DMA transfer start channel number
 * Return Value : None
 ******************************************************************************/
void DMA_SoftwareTrigger(const int_t channel)
{
    /* prevent access to channels below DMA_START_CHANNEL */
    if (channel < DMA_START_CHANNEL)
    {
    ...
    gb_info_drv.info_ch[channel].p_dma_ch_reg->CHCTRL_n = 4;
}
/******************************************************************************
 End of function DMA_SoftwareTrigger
 ******************************************************************************/
 ...
 *           :restart_flag -
 *                  Flag of DMA continuous transfer auto restart.
 * Return Value : None.
 ******************************************************************************/
void DMA_Start(const int_t channel, const bool_t restart_flag)
{
    ...
    if (restart_flag)
    {
        ...
        gb_info_drv.info_ch[channel].p_dma_ch_reg->CHCFG_n &= ~(uint32_t)(CHCFG_SET_RSW | CHCFG_SET_RSEL | CHCFG_SET_REN);
    ...
    R_INTC_Enable(gb_info_drv.info_ch[channel].end_irq_num);
...
void DMA_Stop(const int_t channel, uint32_t * const p_remain)
{
    ...
    R_INTC_Disable(gb_info_drv.info_ch[channel].end_irq_num);
    ...
    *p_remain = gb_info_drv.info_ch[channel].p_dma_ch_reg->CRTB_n;
    ...
    /* clear flag which indicates that next DMA transfer set already */
    gb_info_drv.info_ch[channel].next_dma_flag = false;
    
    /* interrupt clear, if interrupt occurred already */
    R_INTC_ClearPendingInt(gb_info_drv.info_ch[channel].end_irq_num);
...
* Arguments : None.
* Return Value : None.
*                Note: store error code (IOIF_EIO) to AIOCB member.
******************************************************************************/
static void R_DMA_ErrInterruptHandler(void)
{
    ...
    if (NULL != gb_info_drv.p_err_aio)
    {
        ...
        /* set error code (IOIF_EIO) */
        gb_info_drv.p_err_aio->aio_return = IOIF_EIO;
 ...
 * Arguments : None
 * Return Value : None
 *                Note: store error code (IOIF_EIO) to AIOCB
 *                  IOIF_ESUCCESS - DMA transfer completed
 *                  IOIF_EIO - DMA transfer don't stopped
 ******************************************************************************/
static void R_DMA_End0InterruptHandler(void)
{
...
__inline static void R_DMA_EndHandlerProcess(const int_t channel)
{
    ...
    if (NULL != gb_info_drv.info_ch[channel].p_end_aio)
    {
        ...
        if (!store_next_dma_flag)
        {
            ...
            if (0U == (gb_info_drv.info_ch[channel].p_dma_ch_reg->CHSTAT_n & CHSTAT_MASK_EN))
            {
                /* set error code (IOIF_ESUCCESS) */
                gb_info_drv.info_ch[channel].p_end_aio->aio_return = IOIF_ESUCCESS;
            }
            else
            {
                /* set error code (IOIF_EIO) */
                gb_info_drv.info_ch[channel].p_end_aio->aio_return = IOIF_EIO;
            }
        }
        else
        {
